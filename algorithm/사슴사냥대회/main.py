...
def Me(opp, turn, opp_prev, opp_last_pattern):
    me = DEER
    # 일단 상대방이 처음이라면 나는 사슴으로 상대에게 신뢰를 준다.
    if turn <= 2:
        return me
    # 3번정도는 간을 보면서, 사슴 순으로 내고 그 다음부터는 신뢰를 줬으니 사슴을 주지않을까..
    else:
        total_count = 0
        deer_count = 0
        for i in opp_last_pattern[opp]:
            if i == -1:
                break
            total_count += 1
            if i == DEER:
                deer_count += 1
        if deer_count >= total_count * 1/3:
            me = DEER
        else:
            me = SNAKE
    return me
...
def Joo(opp, turn, opp_prev, opp_last_pattern):
    if turn == 0:
        # 만일 상대방의 지난 턴 첫번째 행동이 사슴이면
        if opp_last_pattern[opp][0] == 0:
            # 나도 첫째턴엔 일단 사슴을 냄
            return 0
        # 사슴이 아니면
        else:
